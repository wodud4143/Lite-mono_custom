...
import math
import torch.cuda
from .model_utils import Conv, DepthwiseSeparableConv,InvertedBottleneck,CustomGhostModule,CoordAtt, MLPBlock
...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        if model == 'lite-mono':
            ...
            if height == 192 and width == 640:
                ...
                # self.dilation = [[1, 2, 3], [1, 2, 3], [1, 2, 3, 4, 5, 6]]
        ...
        self.new_stem2 = nn.Sequential(
            CoordAtt(self.dims[0]+3, self.dims[0]+3),
            nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            MLPBlock(dim=self.dims[0]+3),
            nn.Conv2d(self.dims[0]+3, self.dims[0], kernel_size=1)
        )
    ...
    def forward_features(self, x):
        ...
        # Downsample
        x = self.new_stem2(torch.cat((x, x_down[0]), dim=1))
        tmp_x.append(x)
        """------------------------------------"""
