...
class DilatedConv(nn.Module):
    ...
    def __init__(self, dim, k, dilation=1, stride=1, drop_path=0.,
                 layer_scale_init_value=1e-6, expan_ratio=4):
        """
...
class LGFI(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=4,
        ...
        self.gamma_xca = nn.Parameter(layer_scale_init_value * torch.ones(self.dim),
                                      requires_grad=True) if layer_scale_init_value > 0 else None
        self.xca = OptimizedXCA(self.dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
