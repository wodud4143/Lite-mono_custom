...
def evaluate_trt(opt, engine_path=None):
    ...
    # --- [수정됨] 평가 비율에 따라 파일 목록 조절 ---
    eval_ratio = getattr(opt, 'eval_ratio', 1.0)
    if 0.0 < eval_ratio < 1.0:
        print(f"-> 전체 테스트셋의 {eval_ratio:.2%}만 사용하여 평가를 진행합니다.")
        # 재현성을 위해 시드 고정 후 셔플
        random.seed(42)
        random.shuffle(filenames)
        
        num_samples = int(len(filenames) * eval_ratio)
        filenames = filenames[:num_samples]
        print(f"-> 총 {len(filenames)}개의 샘플로 평가합니다.")
    # --- 수정 끝 ---
...
if __name__ == "__main__":
    ...
    p.add_argument('--engine_path', type=str, default=engine_path, help='TensorRT engine 파일 경로 (기본: <load_weights_folder>/model.engine)')
    # --- [수정됨] ---
    p.add_argument('--eval_ratio', type=float, default=1.0, help='평가에 사용할 테스트셋의 비율 (0.0 ~ 1.0)')
