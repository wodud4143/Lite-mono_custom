...
from .mono_dataset_copy import MonoDataset
...
class KITTIDataset(MonoDataset):
    ...
    def get_color(self, folder, frame_index, side, do_flip, do_crop, crop_params, do_cutout, cutout_params):
        ...
        if do_cutout and cutout_params is not None:
            ...
            color_with_cutout = color

        # PIL Image를 numpy array로 변환 (BGR)
        color_np = np.array(color_with_cutout)
        color_np = cv2.cvtColor(color_np, cv2.COLOR_RGB2BGR)
        
        # CLAHE 적용
        color_clahe_np = self.apply_clahe(color_np)
        color_clahe = pil.Image.fromarray(cv2.cvtColor(color_clahe_np, cv2.COLOR_BGR2RGB))
        
        # Shadow 적용
        color_shadow_np = self.apply_shadow(color_np)
        color_shadow = pil.Image.fromarray(cv2.cvtColor(color_shadow_np, cv2.COLOR_BGR2RGB))
        
        return color, color_clahe, color_shadow
